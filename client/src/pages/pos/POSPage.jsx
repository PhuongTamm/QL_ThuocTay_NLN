...
const POSPageInner = () => {
  ...
  // Dùng useEffect theo dõi giỏ hàng
  useEffect(() => {
  ...
  const checkInteractions = async (currentCart) => {
    // Chỉ kiểm tra khi có từ 2 LOẠI thuốc khác nhau trở lên
    const uniqueMedicines = Array.from(
      new Set(currentCart.map((i) => i.medicineId)),
    );
    ...
    try {
      const payloadItems = uniqueMedicines.map((medId) => {
        const invData = inventories.find((inv) => inv.medicineId._id === medId);
        return {
          name: invData.medicineId.name,
          ingredients: invData.medicineId.ingredients,
        };
      });

      const res = await api.post("/medicines/check-interactions", {
        items: payloadItems,
      });
      if (res.data.warning) {
      ...
      console.error("Lỗi check tương tác:", error);
    } finally {
